...
class TraderBot:
    ...
    def _send_매도(self, s_종목코드, dic_포지션, n_초, s_사유, n_현재가=0):
        """ 지정가 매도 주문 송신 (현재가 - N호가, IOC보통) - 보유 전량
            시장가는 호가를 쓸어내려 체결가가 밀리므로 하한을 둔 지정가로 전환.
            지정가라도 최우선 매수호가가 그보다 위면 그 가격에 체결되므로 손해가 아니고, 극단적 미끄러짐만 차단.
            단, 장마감 청산은 미체결 시 오버나이트 리스크가 있어 시장가 유지 (_T_마감시장가) """
        # 수량 확인
        n_수량 = dic_포지션['수량']
        if n_수량 <= 0:
            dic_포지션['상태'] = '없음'
